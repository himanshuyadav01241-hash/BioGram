...
function renderTable(users) {
  ...
  if (!users || users.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8" class="text-center py-4">No registered profiles found.</td></tr>`;
    return;
  }
  ...
  users.forEach((user, index) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <div class="user-cell">
          <img src="${user.avatar || 'https://via.placeholder.com/32'}" class="table-avatar" alt="avatar">
...
if (saveHandleBtn) {
  saveHandleBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (currentOldHandle) {
        await setDoc(doc(db, "handles", currentOldHandle.toLowerCase()), { released: true });
      }

      await setDoc(newHandleRef, { ownerUid: currentReassignUserId });
